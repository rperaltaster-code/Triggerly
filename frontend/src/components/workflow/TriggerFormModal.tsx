...
import { useClients, useClientServices } from '../../hooks/useClients'
...
export function TriggerFormModal({ workflow, formSchema, onClose }: Props) {
  ...
  const { data: clientsResult } = useClients({ pageSize: 200 })
  const { data: clientServices } = useClientServices(selectedClientId || undefined)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!validate()) return
    const execution = await trigger.mutateAsync({
      id: workflow.id,
      inputData: values,
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            {/* Client / Service context (optional) */}
            <div className="space-y-3 pb-4 border-b border-gray-100">
              <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Client Context (optional)</p>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Client</label>
                <select
                  value={selectedClientId}
                  onChange={(e) => { setSelectedClientId(e.target.value); setSelectedServiceId('') }}
                ...
                <div>
                  ...
                  >
                    ...
                    {(clientServices ?? []).map((s) => (
                      <option key={s.id} value={s.id}>{s.serviceTypeName}</option>
                    ))}
                  </select>
                </div>
              )}
